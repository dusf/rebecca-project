...
const promotionListCss = fs.readFileSync(path.join(adminRoot, 'promotion', 'css', 'promotion_list.css'), 'utf8');
...
assert.match(js, /user\.customerNumber/);
assert.match(js, /<div class="um-user-email"[\s\S]*<div class="um-user-id">/);
assert.match(js, /data-row-action="copy-number"/);
assert.match(js, /copied \? '复制成功' : '复制失败'/);
assert.match(js, /\{ key: 'tags', label: '标签', width: 200 \}/);
assert.match(js, /\{ key: 'name', label: '姓名', width: 160, defaultVisible: false \}/);
assert.match(js, /\{ key: 'phone', label: '手机号', width: 180, defaultVisible: false \}/);
assert.match(js, /\{ key: 'preferredLanguage', label: '首选语言', width: 140, defaultVisible: false \}/);
assert.match(js, /\{ key: 'addresses', label: '地址', width: 300, defaultVisible: false \}/);
assert.match(js, /\{ key: 'note', label: '备注', width: 240, defaultVisible: false \}/);
assert.match(js, /function renderAddressCell\(user\)/);
assert.match(js, /function renderNoteCell\(user\)/);
assert.match(js, /tags\.slice\(0, 2\)/);
assert.match(js, /class="um-tag-overflow"/);
assert.doesNotMatch(js, /um-avatar|um-user-name|编号：/);
assert.match(html, /placeholder="搜索客户编号、邮箱、手机号或标签"/);
assert.match(storeJs, /CUSTOMER_SEQUENCE_KEY = 'rebecca_customer_number_sequence_v1'/);
assert.match(storeJs, /return 'CUS-' \+ String\(sequence\)\.padStart\(6, '0'\)/);
assert.match(storeJs, /customerNumber:/);
...
assert.doesNotMatch(js, /data-column-move|um-column-move|使用箭头调整列顺序/);
assert.match(js, /event\.key !== 'ArrowUp' && event\.key !== 'ArrowDown'/);
assert.match(productListHtml, /class="custom-col-header"[\s\S]*data-column-reset[\s\S]*恢复默认/);
assert.match(productListHtml, /class="custom-col-hint">拖拽字段调整列顺序。产品信息始终固定在左侧。/);
assert.doesNotMatch(productListHtml, /custom-col-footer/);
assert.match(commonsJs, /event\.key !== 'ArrowUp' && event\.key !== 'ArrowDown'/);
...
assert.match(dialogHtml, /data-user-dialog=["']export-progress["']/);
assert.match(dialogHtml, /data-user-dialog="csv"[\s\S]*class="um-dialog um-dialog-workflow um-dialog-csv"/);
assert.match(dialogHtml, /data-user-dialog="csv-progress"[\s\S]*class="um-dialog um-dialog-shopify-progress um-dialog-csv-progress"/);
assert.match(dialogHtml, /data-user-dialog="shopify"[\s\S]*class="um-dialog um-dialog-workflow um-dialog-shopify"/);
assert.match(dialogHtml, /data-user-dialog="shopify-progress"[\s\S]*class="um-dialog um-dialog-shopify-progress"/);
...
assert.equal(
  (dialogJs.match(/shopifyStepsMarkup\(\['Shopify 授权', '选择用户'\], [12]\)/g) || []).length,
  3
);
...
assert.match(dialogCss, /\.um-dialog-header h2\s*\{[\s\S]*font-size:\s*16px;[\s\S]*font-weight:\s*700;/);
...
assert.match(dialogCss, /\.um-dialog-csv\s*\{[\s\S]*width:\s*min\(920px,\s*94vw\);[\s\S]*height:\s*min\(660px,\s*calc\(100vh - 48px\)\);/);
assert.match(dialogCss, /\.um-csv-step-upload \.um-upload-zone\s*\{[\s\S]*min-height:\s*280px;[\s\S]*flex:\s*1 1 auto;/);
...
assert.match(adminIndex, /common\/css\/commons\.css\?v=9/);
assert.match(adminIndex, /common\/css\/user_dialogs\.css\?v=26/);
assert.match(adminIndex, /common\/js\/user_dialog\.js\?v=28/);
assert.match(dialogJs, /common\/html\/user_dialogs\.html\?v=14/);
...
assert.match(dialogJs, /实际接入时会跳转到 Shopify 官方登录页/);
assert.match(dialogJs, /data-dialog-action="shopify-other-account"/);
assert.match(dialogJs, /data-dialog-action="shopify-login"/);
assert.match(dialogJs, /data-dialog-action="shopify-authorize"/);
assert.match(dialogJs, /state\.shopify\.step = 2/);
assert.match(dialogJs, /QVR商城后台/);
assert.match(dialogJs, /qvr-official\.myshopify\.com/);
assert.match(dialogJs, /escapeHtml\(record\.email\)/);
assert.doesNotMatch(dialogJs, /escapeHtml\(record\.firstName \+ ' ' \+ record\.lastName\)/);
assert.match(dialogJs, /全选（' \+ records\.length \+ '）/);
assert.match(dialogJs, /class="um-selection-selected">已选 /);
assert.match(dialogCss, /\.um-dialog-combobox-trigger::after\s*\{[\s\S]*stroke='%23A0937D'[\s\S]*stroke-width='2\.5'/);
assert.match(dialogCss, /\.um-shopify-record-state\s*\{[\s\S]*text-align:\s*right;/);
assert.match(dialogJs, /function startShopifyJob\(records, store, hooks\)/);
assert.match(dialogJs, /function processNextShopifyRecord\(job\)/);
assert.match(dialogJs, /getShopifySyncState:/);
assert.match(dialogJs, /openShopifySyncProgress:/);
assert.match(dialogJs, /function startCsvJob\(records, fileName, hooks\)/);
assert.match(dialogJs, /openCsvImportProgress:/);
assert.match(dialogJs, /getCsvImportState:/);
assert.match(dialogJs, /shopifyStepsMarkup\(\['上传文件', '数据预览'\], [12]\)/);
assert.doesNotMatch(dialogJs, /function csvMappingMarkup|renderCsvStep3|csv-review|csv-edit|校验与映射|确认结果/);
assert.match(dialogJs, /closeActive\(true\)/);
assert.match(dialogCss, /data-shopify-view="auth"[\s\S]*height:\s*auto;/);
assert.match(dialogCss, /\.um-shopify-step-selection \.um-shopify-list\s*\{[\s\S]*overflow-y:\s*auto;/);
assert.match(js, /onShopifySyncProgress:/);
assert.match(js, /onCsvImportProgress:/);
assert.match(html, /data-shopify-sync-label>从 Shopify 导入/);
assert.match(html, /data-csv-import-label>从 CSV 导入/);
assert.doesNotMatch(dialogJs, /选择已连接店铺|data-dialog-action="shopify-(?:connect-new|store|records|back-stores)"/);
assert.doesNotMatch(dialogJs, /CONNECTED_STORES|shopifyStores\(\)|selectedStoreId/);
...
assert.match(dialogJs, /function startExportJob\(settings, hooks\)/);
assert.match(dialogJs, /openExportProgress:/);
assert.match(dialogJs, /getExportTaskState:/);
assert.match(js, /prepareExportUsers:/);
assert.match(js, /downloadPreparedExport:/);
assert.match(js, /onExportProgress:/);
...
assert.match(js, /UserStore\.addTagsToUsersLocked/);
...
assert.match(css, /\.um-column-panel\s*\{[\s\S]*width:\s*240px;[\s\S]*overflow:\s*hidden;/);
assert.match(css, /\.um-column-panel \.custom-col-body\s*\{/);
assert.doesNotMatch(css, /\.um-column-move/);
assert.match(commonsCss, /\.custom-col-header\s*\{[\s\S]*display:\s*flex;[\s\S]*justify-content:\s*space-between;/);
assert.match(commonsCss, /\.custom-col-reset\s*\{[\s\S]*font:\s*600 12px\/1\.4/);
assert.match(commonsCss, /\.custom-col-hint\s*\{[\s\S]*font-size:\s*12px;/);
assert.match(commonsCss, /\.custom-col-item\s*\{[\s\S]*font-size:\s*13px;/);
...
assert.match(html, /href="\.\.\/common\/css\/commons\.css\?v=9"/);
assert.match(html, /href="css\/users\.css\?v=28"/);
...
assert.match(html, /src="\.\.\/common\/js\/commons\.js\?v=8"/);
assert.match(html, /href="css\/users\.css\?v=28"/);
assert.match(html, /src="js\/user_store\.js\?v=6"/);
assert.match(html, /src="js\/user_components\.js\?v=6"/);
assert.match(html, /src="js\/users\.js\?v=30"/);
assert.match(js, /const taskActive = \['queued', 'running'\]\.indexOf\(state\.exportTask\.status\) !== -1/);
assert.match(userFormHtml, /href="css\/users\.css\?v=23"/);
assert.match(userFormHtml, /src="js\/user_store\.js\?v=6"/);
assert.match(userFormHtml, /src="js\/user_components\.js\?v=6"/);
assert.match(userFormHtml, /src="js\/user_form\.js\?v=5"/);
...
assert.match(productListHtml, /href="\.\.\/common\/css\/commons\.css\?v=9"/);
assert.match(productListHtml, /\.search-input input::placeholder\s*\{[\s\S]*color:\s*hsl\(var\(--control-placeholder\)\);/);
assert.match(productListHtml, /\.filter-date-trigger\.is-placeholder\s*\{[\s\S]*color:\s*hsl\(var\(--control-placeholder\)\);/);
...
assert.match(js, /badge\(PROVIDER_LABELS\[provider\.type\] \|\| provider\.type, 'provider'\)/);
assert.match(css, /\.um-badge-provider\s*\{[\s\S]*background:\s*hsl\(var\(--primary\) \/ \.1\);[\s\S]*color:\s*hsl\(var\(--primary-700\)\);/);
...
assert.match(html, /<summary class="btn btn-secondary">导出<\/summary>/);
assert.match(html, /<summary class="btn btn-secondary">导入<\/summary>/);
assert.doesNotMatch(html, />导出用户<\/summary>|>导入用户<\/summary>/);
assert.match(html, /导出选中用户<span data-export-count="selected">（0）<\/span>/);
assert.match(css, /\.um-menu-panel \[data-export-count\]\s*\{[\s\S]*margin-left:\s*2px;/);
assert.match(js, /countElement\.textContent = '（' \+ count \+ '）'/);
...
assert.match(js, /um-more-actions um-viewport-menu/);
...
assert.match(dialogJs, /data-dialog-action="marketing-toggle-channel"/);
assert.match(dialogJs, /marketingChannelMarkup\('email', '客户同意接收营销电子邮件。'\)/);
assert.match(dialogJs, /marketingChannelMarkup\('sms', '客户同意接收营销短信。'\)/);
assert.match(dialogJs, /marketingChannelMarkup\('whatsapp', '客户同意接收 WhatsApp 营销消息。'\)/);
assert.doesNotMatch(dialogJs, /name === 'marketing-action'/);
assert.doesNotMatch(dialogJs, /marketing-source|umMarketingNote|marketing-time-toggle/);
assert.match(dialogJs, /source: 'admin'/);
assert.match(dialogJs, /title: settings\.title \|\| '确认删除'/);
assert.doesNotMatch(dialogJs, /确认删除(?:所选)?用户？/);
assert.match(dialogCss, /\[data-user-dialog="delete"\] \.um-dialog\s*\{[\s\S]*width:\s*min\(440px,[\s\S]*padding:\s*24px;/);
assert.match(dialogCss, /\[data-user-dialog="delete"\] \.um-dialog-header\s*\{[\s\S]*padding:\s*0 0 8px;[\s\S]*border-bottom:\s*0;/);
assert.match(dialogCss, /\[data-user-dialog="delete"\] \.um-dialog-body\s*\{[\s\S]*margin-bottom:\s*24px;/);
assert.doesNotMatch(dialogJs, /um-risk-list|有订单的用户：|有 Shopify 绑定的用户：/);
assert.match(userFormJs, /updateMarketingChannel:[\s\S]*setMarketingChannelStatusLocked/);
assert.match(dialogJs, /data-dialog-action="batch-tag-add"/);
assert.match(dialogJs, /data-dialog-action="batch-tag-remove"/);
assert.match(dialogJs, /按 Enter、中文或英文逗号创建标签/);
assert.match(dialogJs, /invokeHookAsync\('addTags', \[state\.batchTag\.ids, tags\]\)/);
assert.match(storeJs, /function addTagsToUsers\(ids, values\)/);
assert.match(storeJs, /function addTagsToUsersLocked\(ids, values\)/);
assert.match(js, /UserStore\.addTagsToUsersLocked\(ids, values\)/);
assert.match(productListHtml, /class="filter-select is-placeholder" id="categoryFilter"/);
assert.match(productListHtml, /class="filter-select is-placeholder" id="statusFilter"/);
assert.match(productListHtml, /class="btn btn-secondary product-secondary-action" id="exportBtn"/);
assert.match(productListHtml, /class="btn btn-secondary product-secondary-action"[^>]*>拉取 Shopify<\/button>/);
assert.doesNotMatch(productListHtml, /拉取shopify<\/button>|hsl\(142 50% 40%\)/);
assert.match(productListHtml, /class="filter-date-control" data-product-date data-label="开始时间"/);
assert.match(productListHtml, /class="filter-date-trigger is-placeholder"[\s\S]*aria-label="开始时间"/);
assert.match(productListHtml, /\.filter-date-trigger\s*\{[\s\S]*path fill='%23A0937D'/);
assert.match(productListHtml, /\.filter-date-popover\s*\{[\s\S]*width:\s*288px;/);
assert.match(commonsCss, /\.searchable-select-trigger\.is-placeholder\s*\{[\s\S]*color:\s*hsl\(var\(--control-placeholder\)\);/);
assert.match(commonsCss, /\.searchable-select\.has-value:hover \.searchable-select-clear/);
assert.match(commonsJs, /className = 'searchable-select-clear'/);
assert.match(commonsJs, /syncSearchableSelectState\(select\)/);
assert.match(css, /\.um-header-actions \.um-file-action\s*\{[\s\S]*background:\s*#fff;/);
assert.match(commonsCss, /\.toast-icon\s*\{[\s\S]*box-shadow:\s*none;/);
assert.match(promotionListCss, /\.filter-date\s*\{[\s\S]*path fill='%23A0937D'/);
assert.match(css, /\.um-date-trigger::after\s*\{[\s\S]*path fill='%23A0937D'/);
[
...
assert.match(js, /ICONS\.delete/);
assert.match(js, /ICONS\.more/);
assert.match(js, /data-row-action="edit"[\s\S]*ICONS\.edit[\s\S]*data-row-action="delete"[\s\S]*ICONS\.delete[\s\S]*um-row-menu/);
assert.match(js, /um-row-menu[\s\S]*data-row-action="marketing"[\s\S]*data-row-action="tag"[\s\S]*data-row-action="copy-email"[\s\S]*data-row-action="account"/);
assert.match(css, /\.um-row-action-danger:hover\s*\{[\s\S]*background:\s*hsl\(0 84% 90%\);[\s\S]*color:\s*hsl\(var\(--error\)\);/);
