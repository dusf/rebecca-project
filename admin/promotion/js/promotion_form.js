...
(function () {
  ...
  // ==================== 静态数据 ====================
  var PF_COLLECTIONS = [
    { id: 'c1', name: '夏季新品', desc: '夏季上新系列' },
    { id: 'c2', name: '电子产品', desc: '数码与配件' },
    { id: 'c3', name: '家居生活', desc: '家居好物' },
    { id: 'c4', name: '美妆个护', desc: '美妆护肤' }
  ];
  var PF_PRODUCTS = [
    { id: 'p1', name: '无线蓝牙耳机', desc: '旗舰降噪', price: '$199.00', skus: [{ id: 'p1s1', label: '黑色', price: '$199.00' }, { id: 'p1s2', label: '白色', price: '$199.00' }] },
    { id: 'p2', name: '智能手表', desc: '运动健康', price: '$299.00', skus: [{ id: 'p2s1', label: '46mm 午夜色', price: '$299.00' }] },
    { id: 'p3', name: '便携咖啡机', desc: '桌面胶囊', price: '$89.00', skus: [{ id: 'p3s1', label: '标准版', price: '$89.00' }] },
    { id: 'p4', name: '瑜伽垫', desc: '防滑加厚', price: '$39.00', skus: [{ id: 'p4s1', label: '紫色', price: '$39.00' }, { id: 'p4s2', label: '灰色', price: '$39.00' }] }
  ];
  var PF_CUSTOMERS = [
    { id: 'cu1', name: '张伟', email: 'zhangwei@example.com' },
    { id: 'cu2', name: '李娜', email: 'lina@example.com' },
    { id: 'cu3', name: '王芳', email: 'wangfang@example.com' },
    { id: 'cu4', name: '刘洋', email: 'liuyang@example.com' },
  ...
  function pfBuildBasicSection() {
    ...
    html += '<div style="margin-top:20px;">';
    html += pfGroup('适用客户',
      '<select class="form-input form-select" id="pfEligibility" onchange="pfOnEligibilityChange()">' +
      '<option value="all">全部客户</option>' +
      '<option value="registered">注册客户</option>' +
      '<option value="subscribed">订阅客户</option>' +
      '<option value="specific">指定客户</option>' +
      '</select>', 'pfEligibilityWrap');
    html += '<div id="pfCustomerWrap" style="' + (pfEligibility === 'specific' ? '' : 'display:none;') + 'margin-top:12px;"></div>';
    html += '</div>';
  ...
  function pfBuildProductAmount() {
    ...
    inner += '<div id="paScopePickWrap" style="' + (PF_SCOPE_STATE.main.type ? '' : 'display:none;') + 'margin-top:12px;" class="pf-scope-trigger">' +
      ...
      '<div class="pf-scope-selected-tags" id="paScopeTags"></div></div>';
    inner += '</div>';
    // 渲染已选标签
  ...
  function pfBuildShipping() {
    ...
    inner += '<div id="shipCountryPickWrap" style="display:none;margin-top:12px;" class="pf-scope-trigger">' +
      ...
      '<div class="pf-scope-selected-tags" id="shipCountryTags"></div></div>';
  ...
  function pfBuildXgy() {
    ...
    inner += '<div id="xgyBuyPickWrap" style="' + (PF_SCOPE_STATE.xgyBuy.type ? '' : 'display:none;') + 'margin-top:12px;" class="pf-scope-trigger">' +
      ...
      '<div class="pf-scope-selected-tags" id="xgyBuyScopeTags"></div></div></div>';
    ...
    inner += '<div id="xgyGiftPickWrap" style="' + (PF_SCOPE_STATE.xgyGift.type ? '' : 'display:none;') + 'margin-top:12px;" class="pf-scope-trigger">' +
      ...
      '<div class="pf-scope-selected-tags" id="xgyGiftScopeTags"></div></div></div>';
  ...
  function pfRebuildBasic() {
    ...
    // 重新设置已选值
    if ($('pfEligibility')) $('pfEligibility').value = pfEligibility;
    if (pfEligibility === 'specific') pfRenderCustomerTags();
  }

  window.pfOnEligibilityChange = function () {
    pfEligibility = $('pfEligibility').value;
    $('pfCustomerWrap').style.display = pfEligibility === 'specific' ? '' : 'none';
    if (pfEligibility === 'specific') pfRenderCustomerTags();
    pfUpdatePreview();
  };
  ...
  window.pfOpenScopeDialog = function (target) {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML =
        ...
        '<div class="pf-dialog-search"><input type="text" id="pfScopeSearch" placeholder="搜索..." oninput="pfOnScopeSearch()"><span class="pf-dialog-select-all" onclick="pfToggleSelectAllScope()">全选</span><span class="pf-dialog-count" id="pfScopeCount2"></span></div>' +
        '<div class="pf-dialog-body"><div class="pf-dialog-list" id="pfScopeList"></div></div>' +
        '<div class="pf-dialog-footer"><button class="btn btn-secondary" onclick="pfCloseScopeDialog()">取消</button><button class="btn btn-primary" onclick="pfConfirmScopeDialog()">确定</button></div>' +
        '</div></div>';
  ...
  function pfRenderScopeDialog() {
    ...
    if (mode === 'collection') {
      PF_COLLECTIONS.forEach(function (c) {
        ...
        html += '<div class="pf-dialog-item' + (sel ? ' selected' : '') + '" onclick="pfToggleScopeCol(\'' + c.id + '\')"><input type="checkbox" ' + (sel ? 'checked' : '') + ' onchange="pfToggleScopeCol(\'' + c.id + '\')"><div class="pf-dialog-item-info"><div class="pf-dialog-item-name">' + esc(c.name) + '</div><div class="pf-dialog-item-desc">' + esc(c.desc) + '</div></div></div>';
      });
    } else {
      PF_PRODUCTS.forEach(function (p) {
        ...
        html += '<div class="pf-dialog-item' + (sel ? ' selected' : '') + '" onclick="pfToggleScopeProduct(\'' + p.id + '\')"><input type="checkbox" ' + (sel ? 'checked' : '') + ' onchange="pfToggleScopeProduct(\'' + p.id + '\')"><div class="pf-dialog-item-info"><div class="pf-dialog-item-name">' + esc(p.name) + '</div><div class="pf-dialog-item-desc">' + esc(p.desc) + '</div></div>' +
          '<button class="pf-dialog-item-expand' + (p._expanded ? ' expanded' : '') + '" onclick="event.stopPropagation();pfToggleSku(\'' + p.id + '\')">▾</button></div>';
        if (p._expanded) {
          p.skus.forEach(function (s) {
            var ssel = (st.products.indexOf(p.id + '|' + s.id) >= 0);
            html += '<div class="pf-dialog-sku-item' + (ssel ? ' selected' : '') + '" onclick="pfToggleSkuSel(\'' + p.id + '\',\'' + s.id + '\')"><input type="checkbox" ' + (ssel ? 'checked' : '') + '><span class="sku-label">' + esc(s.label) + '</span><span class="sku-price">' + esc(s.price) + '</span></div>';
          });
        }
        html += '</div>';
      });
  ...
  window.pfToggleScopeProduct = function (id) {
    var st = PF_SCOPE_STATE[PF_SCOPE_TARGET];
    var idx = st.products.indexOf(id);
    if (idx >= 0) { st.products.splice(idx, 1); } else { st.products.push(id); }
    pfRenderScopeDialog();
  };
  ...
  function pfRenderScopeTags(target, tagsId, countId) {
    ...
    var tagsEl = $(tagsId); if (!tagsEl) return;
    var names = [];
    st.collections.forEach(function (id) { var c = PF_COLLECTIONS.find(function (x) { return x.id === id; }); if (c) names.push(c.name); });
    st.products.forEach(function (id) {
      if (id.indexOf('|') >= 0) { var parts = id.split('|'); var p = PF_PRODUCTS.find(function (x) { return x.id === parts[0]; }); var s = p && p.skus.find(function (y) { return y.id === parts[1]; }); if (p) names.push(p.name + (s ? ' / ' + s.label : '')); }
      else { var p2 = PF_PRODUCTS.find(function (x) { return x.id === id; }); if (p2) names.push(p2.name); }
    });
    tagsEl.innerHTML = names.map(function (n) { return '<span class="pf-scope-tag">' + esc(n) + '</span>'; }).join('');
    var c = $(countId); if (c) c.textContent = names.length ? '(' + names.length + ')' : '';
  }
  ...
  window.pfOpenCountryDialog = function () {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML =
        ...
        '<div class="pf-dialog-search"><input type="text" id="pfCountrySearch" placeholder="搜索国家..." oninput="pfRenderCountryDialog()"><span class="pf-dialog-select-all" onclick="pfToggleSelectAllCountry()">全选</span><span class="pf-dialog-count" id="pfCountryCount2"></span></div>' +
        '<div class="pf-dialog-body"><div class="pf-dialog-list" id="pfCountryList"></div></div>' +
        '<div class="pf-dialog-footer"><button class="btn btn-secondary" onclick="pfCloseCountryDialog()">取消</button><button class="btn btn-primary" onclick="pfConfirmCountryDialog()">确定</button></div></div>';
      document.body.appendChild(overlay);
    }
    overlay.style.display = 'flex';
  ...
  window.pfToggleCountry = function (id) {
    ...
    pfRenderCountryDialog();
    var tags = $('shipCountryTags');
    if (tags) { tags.innerHTML = pfSelectedCountries.map(function (id2) { var cc = PF_COUNTRIES.find(function (x) { return x.id === id2; }); return cc ? '<span class="pf-scope-tag">' + esc(cc.name) + '</span>' : ''; }).join(''); }
    var cnt = $('shipCountryCount'); if (cnt) cnt.textContent = pfSelectedCountries.length ? '(' + pfSelectedCountries.length + ')' : '';
  };
  ...
  window.pfOpenCustomerDialog = function () {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML =
        '<div class="pf-dialog"><div class="pf-dialog-header"><div class="pf-dialog-title">选择指定客户</div><button class="pf-dialog-close" onclick="pfCloseCustomerDialog()">✕</button></div>' +
        '<div class="pf-dialog-search"><input type="text" id="pfCustomerSearch" placeholder="搜索客户..." oninput="pfRenderCustomerDialog()"><span class="pf-dialog-select-all" onclick="pfToggleSelectAllCustomer()">全选</span><span class="pf-dialog-count" id="pfCustomerCount2"></span></div>' +
        '<div class="pf-dialog-body"><div class="pf-dialog-list" id="pfCustomerList"></div></div>' +
        '<div class="pf-dialog-footer"><button class="btn btn-secondary" onclick="pfCloseCustomerDialog()">取消</button><button class="btn btn-primary" onclick="pfConfirmCustomerDialog()">确定</button></div></div>';
      document.body.appendChild(overlay);
    }
    overlay.style.display = 'flex';
  ...
  function pfRenderCustomerDialog() {
    ...
    var list = $('pfCustomerList');
    list.innerHTML = PF_CUSTOMERS.map(function (c) {
      ...
      return '<div class="pf-dialog-item' + (sel ? ' selected' : '') + '" onclick="pfToggleCustomer(\'' + c.id + '\')"><input type="checkbox" ' + (sel ? 'checked' : '') + ' onchange="pfToggleCustomer(\'' + c.id + '\')"><div class="pf-dialog-item-info"><div class="pf-dialog-item-name">' + esc(c.name) + '</div><div class="pf-dialog-item-desc">' + esc(c.email) + '</div></div></div>';
    }).join('');
  ...
  window.pfToggleSelectAllCustomer = function () {
    if (pfSelectedCustomers.length === PF_CUSTOMERS.length) pfSelectedCustomers = []; else pfSelectedCustomers = PF_CUSTOMERS.map(function (c) { return c.id; });
    pfRenderCustomerDialog();
  };
  window.pfConfirmCustomerDialog = function () {
    pfCloseCustomerDialog();
    pfRenderCustomerTags();
    pfUpdatePreview();
  };
  function pfRenderCustomerTags() {
    var wrap = $('pfCustomerWrap');
    if (!wrap || pfEligibility !== 'specific') return;
    var names = pfSelectedCustomers.map(function (id) { var c = PF_CUSTOMERS.find(function (x) { return x.id === id; }); return c ? c.name : ''; }).filter(Boolean);
    wrap.innerHTML = '<button type="button" class="pf-scope-btn" onclick="pfOpenCustomerDialog()">选择客户 <span>(' + names.length + ')</span></button>' +
      '<div class="pf-scope-selected-tags">' + names.map(function (n) { return '<span class="pf-scope-tag">' + esc(n) + '</span>'; }).join('') + '</div>';
  }
  ...
  window.pfSave = function () {
    ...
    setTimeout(function () { if (window.parent.goBack) window.parent.goBack(); else window.history.back(); }, 700);
  };
  ...
  window.pfPopulateForm = function (promo) {
    ...
    if (pfCurrentType === 'order_amount') {
      ...
      if (promo.countryType === 'specific') { var tags = $('shipCountryTags'); if (tags) tags.innerHTML = pfSelectedCountries.map(function (id2) { var cc = PF_COUNTRIES.find(function (x) { return x.id === id2; }); return cc ? '<span class="pf-scope-tag">' + esc(cc.name) + '</span>' : ''; }).join(''); var cnt = $('shipCountryCount'); if (cnt) cnt.textContent = pfSelectedCountries.length ? '(' + pfSelectedCountries.length + ')' : ''; }
  ...
  window.pfGoBack = function () { if (window.parent && window.parent.goBack) window.parent.goBack(); else window.history.back(); };
})();
